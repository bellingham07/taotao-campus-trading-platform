...
func StartCcConsumer(conn *amqp.Connection) {
	...
	r := utils.NewRabbitMQ(utils.AtclCollectDeadQueue, utils.AtclCollectDeadExchange, "ac", conn, channel)
	...
	// 开始监听
	msgs, err := r.Channel.Consume(utils.AtclCollectDeadQueue, "", false, false, false, false, nil)
	if err != nil {
		panic(err)
	}
...
func StartCLConsumer(conn *amqp.Connection) {
	channel, err := conn.Channel()
	if err != nil {
		panic("[RABBITMQ ERROR] 初始化 cmdty collect consumer 错误！" + err.Error())
	}
	r := utils.NewRabbitMQ(utils.AtclLikeQueue, utils.AtclLikeExchange, "cl", conn, channel)
	...
	// 开始监听
	msgs, err := r.Channel.Consume(utils.AtclLikeQueue, "", false, false, false, false, nil)
	if err != nil {
		panic(err)
	}
	forever := make(chan int, 0)
	for msg := range msgs {
		logx.Infof("接受成功咕咕咕咕咕咕过过过过过过过过过过过")
		lMessage := new(utils.LMessage)
		err = Json.Unmarshal(msg.Body, lMessage)
		if err != nil {
			logx.Infof("[JSON UNMARSHAL ERROR] StartCLConsumer json解析失败 %v\n", err)
			msg.Nack(false, false)
			continue
		}
