...
type (
	TradeInfo struct {
		...
		IsSellerConfirmed time.Time `xorm:"is_seller_confirmed" json:"isSellerConfirmed"`
		IsBuyerConfirmed  time.Time `xorm:"is_buyer_confirmed" json:"isBuyerConfirmed"`
	...
	TradeCmt struct {
		Id         int64     `xorm:"id"` // id
		TradeId    int64     `xorm:"trade_id" json:"tradeId"`
		UserId     int64     `xorm:"user_id" json:"userId"`
		User       string    `xorm:"user" json:"user"`
		UserAvatar string    `xorm:"user_avatar" json:"userAvatar"`
		ToUserId   int64     `xorm:"to_user_id" json:"toUserId"`
		Content    string    `xorm:"content" json:"content"`    // 评价内容
		Type       int64     `xorm:"type" json:"type"`          // 差评或好评，0为差评，1为好评
		CreateAt   time.Time `xorm:"create_at" json:"createAt"` // 创建时间
	}
)
